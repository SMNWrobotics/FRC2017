#tag Window
Begin WindowMenuWindow winStartup
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   True
   Compatibility   =   ""
   ...
   Begin Label Label1
      ...
      Width           =   396
   End
   Begin BevelButton bbImportMatchData
      AcceptFocus     =   True
      AutoDeactivate  =   True
      BackColor       =   &c00000000
      Bevel           =   0
   ...
   Begin Label Label2
      ...
      Width           =   396
   End
   Begin BevelButton bbExportMatchData
      AcceptFocus     =   True
      AutoDeactivate  =   True
      BackColor       =   &c00000000
      Bevel           =   0
...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  me.AcceptFileDrop(FileTypes1.SQLite)
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Sub Init()
		  //Check to see if we have any events, teams, matches
		  ...
		  if moEvent = nil or moEvent.ID = 0 then
		    ...
		    bbScout.Enabled = false
		    bbList.Enabled = false
		    lblCustomized.text = kLabel.replace("^1", "unknown")
		    ...
		    bbList.Enabled = true
		    lblCustomized.text = kLabel.replace("^1", moEvent.sShort_Name + " at " + moEvent.sLocation)
		    bbReporting.Enabled = true
		    bbRobot.Enabled = true
		  end
		End Sub
	#tag EndMethod
	...
	#tag Constant, Name = kLabel, Type = String, Dynamic = False, Default = \"Scout a match at ^1.  Keep track of one robot in a match and score how it does.", Scope = Public
	#tag EndConstant
...
#tag Events bbList
	...
	#tag EndEvent
#tag EndEvents
#tag Events bbReporting
	#tag Event
		Sub Action()
		  dim w as new winReporting
